...
| **日給・日当での募集**（`pay_unit = daily`） | 掲載する。日給は単発の仕事の指標 |
...
**日給を指標にする理由**: 日給・日当での募集は単発の仕事である、という依頼者の判断。
これは Source に書かれた事実（報酬の単位）からの判断であって推論ではない。

注意: 「日給月給制」は建設業などの**正社員**の給与制度なので、これに含めてはいけない。
`extractPay` は単位語の直後に金額が来ることを要求するため「日給月給250,000円」を
daily と判定しない。判定は本文の文字列ではなく `pay_unit` を見る。
正社員の記載と日給が同居する場合（日給制の正社員）は `review_required` になる。

**時給を指標にしない理由**: 時給は契約社員でも使われるため、単独では副業の指標にならない。
